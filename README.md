...
- The frontend sends requests to the Cloudflare Worker at `/api/chat` with **no API key attached**.
- The Worker injects the `DEEPSEEK_API_KEY` (stored as a Cloudflare secret) and forwards the request to DeepSeek.
- CORS is restricted to the GitHub Pages origin (`https://wangyangmingsss.github.io`).
- A simple per-IP rate limiter (10 requests/minute) prevents abuse.
